...
def determine_winner(insta1, insta2, as_string):
    ...
    Returns the dict with the larger follower_count, or if T, a string of 'a' or 'b', depending on the follower count."""
...
def game():
    ...
    while game_over is False:
        ...
        user_choice = input("Who has more instragram followers? Type 'a' or 'b'. ")
        # Ends the game on wrong choice
        if user_choice is not more_followers:
            print(f"Thats incorrect! {a['name']} has {a['follower_count']} followers, and {b['name']} has {b['follower_count']} followers.")
            game_over = True
            print(f"You have lost with a score of {score}.")
            return
        else:
            print(f"Thats correct! {a['name']} has {a['follower_count']} followers, and {b['name']} has {b['follower_count']} followers.")
            score += 1
